...
def encryptor():
    ...
    private_key = hashlib.sha256(privateKeyOfSenderEntry.get().encode()).digest()
...
def main():
	...
	-------------------------------------------------------------
	'''
	EncryptStep = tkinter.LabelFrame(form, text=" 1. File Encryption: ")
	...
	#-------------------------Encrypt Button---------------------
	EncryptBTN = Button(EncryptStep, text="Encrypt   ", command=encryptor)
	...
	#-------------------------Decrypt Button---------------------
	DecryptBTN = Button(DecryptStep, text="Decrypt   ", command = decryptor)
