...
def generate_private_key(length):
    rand = 0
    bytes_needed = length // 8 + 8
    
    # Generate a random private key such that it's less than the prime number
    while rand.bit_length() < length:
        # Generate random bytes and convert to denary format
        hex_key = binascii.b2a_hex(os.urandom(bytes_needed))
        rand = int(hex_key, 16)
...
def generate_secret(private_key, public_key):
    ...
    # Convert the secret to bytes and hash it using SHA256
